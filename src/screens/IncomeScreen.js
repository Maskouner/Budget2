...
const IncomeScreen = () => {
  ...
  const addIncomes = async ({ amount, date, category, comment }) => {
    await firestore().collection('Utilisateurs').doc('18c79361-d05f-437b-9909-685db8d4910a').collection('Incomes').add({
      amount: '€'+amount,
      date: date,
      category: category,
      comments: comment,
